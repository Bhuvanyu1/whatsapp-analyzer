...
export class NLPProcessor {
  ...
  private static extractKeywords(text: string): ExtractedKeyword[] {
    ...
    return keywords.sort((a, b) => b.frequency - a.frequency).slice(0, 50); // Top 50 keywords
  }
    ...
    this.TECHNOLOGY_PATTERNS.forEach((pattern) => {
      ...
      if (matches) {
        matches.forEach((match) => {
          const cleaned = match
            .replace(
              /\b(?:using|with|in|on|via|through|built|created|developed|worked|expert|experienced|skilled)\s+/gi,
              "",
            )
  ...
  private static categorizeKeyword(
    keyword: string,
  ): "skill" | "technology" | "industry" | "role" | "company" | "general" {
    const lowerKeyword = keyword.toLowerCase();

    // Check against known skill keywords
    if (
      this.SKILL_KEYWORDS.some(
        (skill) => lowerKeyword.includes(skill) || skill.includes(lowerKeyword),
      )
    ) {
      return lowerKeyword.includes("management") ||
        lowerKeyword.includes("manager")
        ? "role"
        : "skill";
    }
    ...
    if (
      lowerKeyword.match(
        /\b(js|css|html|api|sdk|framework|library|database|server|cloud)\b/,
      )
    ) {
      return "technology";
    }
    ...
    if (
      lowerKeyword.match(
        /\b(manager|director|lead|senior|junior|intern|ceo|cto|cfo|developer|engineer|designer)\b/,
      )
    ) {
      return "role";
    }
    ...
    if (
      lowerKeyword.match(
        /\b(healthcare|finance|education|retail|manufacturing|consulting|tech|startup)\b/,
      )
    ) {
      return "industry";
    }
    ...
    if (
      lowerKeyword.match(
        /\b(corp|inc|ltd|llc|company|technologies|solutions|systems)\b/,
      ) ||
      (keyword.charAt(0) === keyword.charAt(0).toUpperCase() &&
        keyword.length > 4)
    ) {
      return "company";
    }
    ...
    helpPatterns.forEach((pattern) => {
      ...
      if (matches) {
        matches.forEach((match) => {
          const skill = match
            .replace(
              /\b(?:help|assist|support|with|you|in|i know|familiar|experience|good at)\s+/gi,
              "",
            )
            .trim()
            .toLowerCase();
  ...
  private static analyzeSentiment(
    ...
    messages.forEach((message) => {
      if (!message.isSystemMessage && message.content.length > 5) {
        ...
        if (tokens) {
          ...
          if (
            /\b(help|assist|support|advice|suggest|recommend)\b/i.test(
              message.content,
            )
          ) {
            helpfulnessScore += 1;
          ...
          if (
            /[!]{1,3}|great|awesome|excited|love|amazing/i.test(message.content)
          ) {
            enthusiasmScore += 1;
          }
    ...
    return {
      ...
      helpfulness:
        messageCount > 0 ? Math.min(helpfulnessScore / messageCount, 1) : 0,
      enthusiasm:
        messageCount > 0 ? Math.min(enthusiasmScore / messageCount, 1) : 0,
    };
  }

  private static analyzeCommunicationStyle(
    messages: ParsedMessage[],
  ): ContactAnalysis["communicationStyle"] {
    ...
    const avgLength =
      ...
      userMessages.length;
    const messageLength =
      avgLength < 50 ? "short" : avgLength > 150 ? "long" : "medium";

    // Analyze formality based on patterns
    const formalPatterns =
      /\b(please|thank you|regards|sincerely|would|could|may i)\b/gi;
    const informalPatterns = /\b(hey|hi|yeah|yep|cool|awesome|lol|haha)\b/gi;

    const formalCount = userMessages.reduce(
      (count, m) => count + (m.content.match(formalPatterns) || []).length,
      0,
    );
    const informalCount = userMessages.reduce(
      (count, m) => count + (m.content.match(informalPatterns) || []).length,
      0,
    );
  ...
  static calculateRelevance(
    ...
    const queryTokens =
      new natural.WordTokenizer().tokenize(query.toLowerCase()) || [];
    ...
    contactAnalysis.expertise.forEach((exp) => {
      queryKeywords.forEach((queryToken) => {
        if (
          exp.skill.toLowerCase().includes(queryToken) ||
    ...
    contactAnalysis.keywords.forEach((keyword) => {
      queryKeywords.forEach((queryToken) => {
        if (
          keyword.keyword.toLowerCase().includes(queryToken) ||
          queryToken.includes(keyword.keyword.toLowerCase())
        ) {
          relevanceScore +=
            keyword.confidence * (keyword.category === "skill" ? 1.5 : 1);
        }
      });
    });
