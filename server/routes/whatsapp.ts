...
export const validateFile: RequestHandler = async (req, res) => {
  try {
    ...
    const metadata = WhatsAppParser.extractMetadata(
      content,
...
export const importFile: RequestHandler = async (req, res) => {
  try {
    ...
    const parsedChat = WhatsAppParser.parseContent(
      content,
      fileName,
      req.file.size,
    );
    ...
    const importResult =
      await getWhatsappImportService().processImport(parsedChat);

    // Step 3: Run NLP analysis on contacts
    let expertiseCount = 0;
    for (const participant of parsedChat.participants) {
      const userMessages = parsedChat.messages.filter(
        (m) => m.sender === participant.name && !m.isSystemMessage,
      );

      if (userMessages.length > 5) {
        // Only analyze contacts with enough messages
        const analysis = NLPProcessor.analyzeContact(userMessages);

        // Store expertise in database
        const contact = getContactModel().findByNormalizedName(
...
export const exportData: RequestHandler = async (req, res) => {
  try {
    ...
    if (format === "csv" && type === "contacts") {
      ...
      const csvHeader =
        "name,company,role,location,total_messages,relationship_strength,trust_level\n";
      const csvData = data
        .map(
          (contact: any) =>
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${filename.replace(".json", ".csv")}"`,
      );
