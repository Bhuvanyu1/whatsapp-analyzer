...
const ContactQuerySchema = z.object({
  ...
  sortBy: z
    .enum(["name", "lastContact", "messageCount", "relationshipStrength"])
    .optional()
    .default("lastContact"),
  ...
  try {
    ...
    if (query.trustLevel) {
      filteredContacts = filteredContacts.filter(
        (c) => c.trust_level === query.trustLevel,
      );
    }
...
export const getContact: RequestHandler = async (req, res) => {
  try {
    ...
    const messagesByMonth = recentMessages.reduce(
      (acc, msg) => {
    ...
    const enrichedContact = {
      ...
      statistics: {
        ...
        messagesByMonth,
        avgMessagesPerMonth:
          Object.values(messagesByMonth).length > 0
            ? Math.round(
                Object.values(messagesByMonth).reduce((a, b) => a + b, 0) /
                  Object.values(messagesByMonth).length,
              )
            : 0,
        conversationSpan:
          contact.first_contact_date && contact.last_contact_date
            ? Math.ceil(
                (new Date(contact.last_contact_date).getTime() -
                  new Date(contact.first_contact_date).getTime()) /
                  (1000 * 60 * 60 * 24),
              )
            : 0,
      },
...
export const searchNetwork: RequestHandler = async (req, res) => {
  try {
    ...
    for (const contact of allContacts) {
      ...
      const relevanceScore = NLPProcessor.calculateRelevance(
        query,
        mockAnalysis,
      );
...
export const getNetworkAnalytics: RequestHandler = async (req, res) => {
  try {
    ...
    const networkHealth = {
      ...
      avgRelationshipStrength:
        allContacts.length > 0
          ? Math.round(
              (allContacts.reduce(
                (sum, c) => sum + c.relationship_strength,
                0,
              ) /
                allContacts.length) *
                10,
    ...
    const trustDistribution = allContacts.reduce(
      (acc, contact) => {
...
export const recordSearchFeedback: RequestHandler = async (req, res) => {
  try {
    ...
    if (contactId) {
      getSearchQueryModel().updateFeedback(
        searchId,
        parseInt(contactId),
        feedback || "clicked",
      );
