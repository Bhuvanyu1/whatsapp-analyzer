  ...
  const processFile = async (fileInfo: UploadedFile, file: File) => {
    try {
      ...
      updateFile(fileInfo.id, {
        status: "error",
        error:
          "Failed to process file. Please ensure it's a valid WhatsApp export.",
      });
    }
  };
  ...
  const parseWhatsAppFile = (content: string) => {
    ...
    messages.forEach((message) => {
      ...
      if (match) {
        ...
        if (
          contact.includes("added") ||
          contact.includes("left") ||
          contact.includes("changed")
        ) {
          isGroupChat = true;
        } else {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload your WhatsApp chat exports (.txt files) to build your
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2 text-sm">
                        <div>
                          ...
                          <span className="ml-1 font-medium">
                            {file.parsedData.totalMessages.toLocaleString()}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">
                          ...
                          <span className="ml-1 font-medium">
                            {file.parsedData.uniqueContacts}
                          </span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Type:</span>
                          <span className="ml-1 font-medium">
                            {file.parsedData.isGroupChat
                              ? "Group"
                              : "Individual"}
                          </span>
                        </div>
                        <div>
                          ...
                          <span className="ml-1 font-medium text-xs">
                            {file.parsedData.dateRange.start} -{" "}
                            {file.parsedData.dateRange.end}
                          </span>
                        </div>
                      </div>
                    )}
      ...
      {totalStats.files > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Total data processed from all uploads
            </CardDescription>
          </CardHeader>
          <CardContent>
